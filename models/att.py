    ...
    def __init__(
        ...
        *,
        ...
        if self.args.position_and_content:
            print('Using position and content')
        elif self.args.position_only:
            print('Using position only')
        else:
            print('no positional embedding')
    def forward(self, fmap):
        heads, b, c, h, w = self.heads, *fmap.shape

        q, k = self.to_qk(fmap).chunk(2, dim=1)  # [B, heads*128, H/8, W/8]  [B, heads*128, H/8, W/8]

        q, k = map(lambda t: rearrange(t, 'b (h d) x y -> b h x y d', h=heads), (q, k))  # [B, heads, H/8, W/8, 128]  [B, heads, H/8, W/8, 128]
        q = self.scale * q
...
class Aggregate(nn.Module):
    ...
    def forward(self, attn, fmap):
        ...
        v = self.to_v(fmap)  # [B, heads*128, H/8, W/8]
        v = rearrange(v, 'b (h d) x y -> b h (x y) d', h=heads)  # [B, heads, H/8*W/8, 128]
        out = einsum('b h i j, b h j d -> b h i d', attn, v)  # [B, heads, H/8*W/8, 128]
        out = rearrange(out, 'b h (x y) d -> b (h d) x y', x=h, y=w)  # [B, heads*128, H/8, W/8]
